...
namespace DotNetBlazor.Server.Utility.Helpers
{
    ...
    public partial class ApiException : Exception
    {
        ...
        public ApiException(string message, int statusCode, string response, IReadOnlyDictionary<string, IEnumerable<string>> headers, Exception innerException)
            : base(message + ", Status: " + statusCode + ", Response: " + response.Substring(0, response.Length >= 512 ? 512 : response.Length), innerException)
        {
    ...
    public class CustomValidationException : Exception
    {
        ...
        public CustomValidationException(string message, List<ValidationError> validationErrors)
            : base(message)
        {
